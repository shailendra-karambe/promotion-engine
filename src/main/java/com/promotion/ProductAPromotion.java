...
/**
 * This class represents the promotion applied to Product of type A
 * @author Shailendra
 *
 */
public class ProductAPromotion implements Promotion {

    private final String name;
    private final boolean isActive;

    public ProductAPromotion(boolean isActive) {
        this.name = "Promotion on A";
    ...
    public Optional<PromotionResult> apply(List<Item> items) {
        if (items != null && !items.isEmpty()) {
            int discountedPrice = 130;
            int howManyQuantity = 3;
            //Get the Item of product type A, will be one Item
            Optional<Item> applicableItem = items.stream().filter(itm -> itm.getProduct() == Product.A).findFirst();
